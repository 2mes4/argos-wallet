...
func SetupTestApp(t *testing.T) *TestApp {
	...
	dbURL := "postgres://dev:dev@localhost:5432/argos_test?sslmode=disable"
...
func (a *TestApp) DoJSON(method, path string, body interface{}) (int, interface{}) {
...
func (a *TestApp) DoMap(method, path string, body interface{}) (int, map[string]interface{}) {
	status, data := a.DoJSON(method, path, body)
	if data == nil {
		return status, nil
	}
...
func (a *TestApp) DoList(method, path string, body interface{}) (int, []interface{}) {
	status, data := a.DoJSON(method, path, body)
	if data == nil {
		return status, nil
	}
...
func (a *TestApp) CreateWallet(t *testing.T) map[string]interface{} {
	status, wallet := a.DoMap("POST", "/v1/wallets", map[string]interface{}{
...
func TestWalletCreateAndGet(t *testing.T) {
	...
	status, fetched := app.DoMap("GET", "/v1/wallets/"+walletID, nil)
...
func TestWalletWithExternalID(t *testing.T) {
	...
	status, wallet := app.DoMap("POST", "/v1/wallets", map[string]interface{}{
...
func TestWalletDeactivate(t *testing.T) {
	...
	status, _ := app.DoMap("DELETE", "/v1/wallets/"+walletID, nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestWalletGetAddresses(t *testing.T) {
	...
	status, addrs := app.DoList("GET", "/v1/wallets/"+walletID+"/addresses", nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestLinkExternalWallet(t *testing.T) {
	...
	status, conn := app.DoMap("POST", fmt.Sprintf("/v1/wallets/%s/connections", walletID), map[string]interface{}{
		"provider": "metamask",
...
func TestListExternalConnections(t *testing.T) {
	...
	app.DoMap("POST", fmt.Sprintf("/v1/wallets/%s/connections", walletID), map[string]interface{}{
		"provider": "metamask",
		"address":  "0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
		"chain_id": 137,
	})

	status, conns := app.DoList("GET", fmt.Sprintf("/v1/wallets/%s/connections", walletID), nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestUnlinkExternalWallet(t *testing.T) {
	...
	_, conn := app.DoMap("POST", fmt.Sprintf("/v1/wallets/%s/connections", walletID), map[string]interface{}{
		"provider": "metamask",
		"address":  "0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
		"chain_id": 137,
	})
	...
	status, _ := app.DoMap("DELETE", fmt.Sprintf("/v1/wallets/%s/connections/%s", walletID, connID), nil)
	assert.Equal(t, http.StatusOK, status)

	_, conns := app.DoList("GET", fmt.Sprintf("/v1/wallets/%s/connections", walletID), nil)
...
func TestCreateSweepRule(t *testing.T) {
	...
	status, rule := app.DoMap("POST", "/v1/routing/rules", map[string]interface{}{
		"wallet_id": walletID,
...
func TestListRoutingRules(t *testing.T) {
	...
	app.DoMap("POST", "/v1/routing/rules", map[string]interface{}{
		"wallet_id":  walletID,
		"name":       "Rule 1",
		"type":       "sweep",
	...
	status, rules := app.DoList("GET", "/v1/routing/rules?wallet_id="+walletID, nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestDeleteRoutingRule(t *testing.T) {
	...
	_, rule := app.DoMap("POST", "/v1/routing/rules", map[string]interface{}{
		"wallet_id":  walletID,
		"name":       "To delete",
		"type":       "sweep",
	...
	status, _ := app.DoMap("DELETE", "/v1/routing/rules/"+ruleID, nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestExecuteRoutingRule(t *testing.T) {
	...
	status, exec := app.DoMap("POST", "/v1/routing/rules/"+ruleID+"/execute", nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestRuleExecutions(t *testing.T) {
	...
	app.DoMap("POST", "/v1/routing/rules/"+ruleID+"/execute", nil)

	status, execs := app.DoList("GET", "/v1/routing/rules/"+ruleID+"/executions", nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestIdentitySignMessage(t *testing.T) {
	...
	status, resp := app.DoMap("POST", "/v1/identity/sign-message", map[string]interface{}{
		"wallet_id": walletID,
		"message":   "Hello World",
	})
...
func TestIdentityVerifySignature(t *testing.T) {
	...
	_, signResp := app.DoMap("POST", "/v1/identity/sign-message", map[string]interface{}{
		"wallet_id": walletID,
		"message":   "Verify me",
	})
	...
	status, verifyResp := app.DoMap("POST", "/v1/identity/verify-signature", map[string]interface{}{
		"message":   "Verify me",
		"signature": signature,
		"address":   address,
	})
...
func TestIdentityVerifyInvalidSignature(t *testing.T) {
	...
	status, _ := app.DoMap("POST", "/v1/identity/verify-signature", map[string]interface{}{
...
func TestListTransactionsEmpty(t *testing.T) {
	...
	status, txs := app.DoList("GET", "/v1/transactions", nil)
	assert.Equal(t, http.StatusOK, status)
...
func TestListTransactionsWithFilters(t *testing.T) {
	...
	status, txs := app.DoList("GET", "/v1/transactions?wallet_id="+walletID+"&limit=10", nil)
	assert.Equal(t, http.StatusOK, status)
	assert.Equal(t, 0, len(txs))
}
...
func TestMultiTenantIsolation(t *testing.T) {
	...
	_, apiKey2, err := app.TenantSvc.Register(context.Background(), domain.CreateTenantRequest{
		Name: "Tenant 2",
		Plan: "starter",
	})
	require.NoError(t, err)
...
func TestFullWorkflow(t *testing.T) {
	...
	status, _ = app.DoMap("POST", fmt.Sprintf("/v1/wallets/%s/connections", walletID), map[string]interface{}{
		"provider": "metamask",
		"address":  "0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
		"chain_id": 137,
	})
	...
	status, signResp := app.DoMap("POST", "/v1/identity/sign-message", map[string]interface{}{
		"wallet_id": walletID,
	...
	status, txs := app.DoList("GET", "/v1/transactions?wallet_id="+walletID, nil)
	assert.Equal(t, http.StatusOK, status)
