...
```bash
npm install @open-wallet/core @open-wallet/storage-memory
```
...
npm install @open-wallet/network-polygon

# Base (Coinbase L2)
npm install @open-wallet/network-base

# Ethereum mainnet
npm install @open-wallet/network-ethereum
```
...
```bash
# Europe (SEPA)
npm install @open-wallet/regional-sepa

# Americas (ACH, SPEI)
npm install @open-wallet/regional-americas
```
...
import { MemoryStorage } from '@open-wallet/storage-memory';
import { createPolygonProvider } from '@open-wallet/network-polygon';
import { SepaFiatProvider } from '@open-wallet/regional-sepa';
...
<body>
  ...
  >
    <open-wallet-card></open-wallet-card>
...
```tsx
import { useOpenWallet, WalletCard, TransactionList } from '@open-wallet/react';

function Dashboard({ walletId }: { walletId: string }) {
  const { balances, loading, fetchTransactions } = useOpenWallet(
...
```dart
import 'package:open_wallet/open_wallet.dart';
