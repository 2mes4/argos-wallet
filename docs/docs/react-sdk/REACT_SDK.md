# React SDK — `@open-wallet/react`

React hooks and components for integrating Open Wallet into React 18+ applications. The SDK wraps the wallet REST API and provides ready-to-use UI components.
...
```tsx
import { useOpenWallet } from '@open-wallet/react';
...
```tsx
import { WalletCard } from '@open-wallet/react';
...
```tsx
import { ConnectWalletButton } from '@open-wallet/react';
...
```tsx
import { TransactionList } from '@open-wallet/react';
...
import {
  ...
  ConnectWalletButton,
  TransactionList,
} from '@open-wallet/react';

function WalletDashboard({ walletId }: { walletId: string }) {
  const { balances, loading, transactions, fetchTransactions } = useOpenWallet(
