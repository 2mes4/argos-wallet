...
The SDK separates business logic from presentation. The core engine (`@open-wallet/core`) has zero UI dependencies and can run in any Node.js environment. The UI components (`@open-wallet/ui`) are an optional layer.
...
```
┌─────────┐     ┌─────────────┐     ┌──────────────┐     ┌─────────────┐
│  UI      │────→│  API Route  │────→│  OpenWallet   │────→│  Blockchain  │
...
└──────────┬──────────────┬──────────────┬─────────────────────────┘
     ...
     │  (Lit)     │ │ useOpen   │ │  WalletApi    │
     │            │ │ Wallet()  │ │  Service()    │
     │ @open-     │ │ @open-    │ │  open_wallet  │
...
|----------|---------|-------------|
| Web Components | `@open-wallet/ui` | Multi-framework sites, plain HTML, SSR |
| React | `@open-wallet/react` | React/Next.js apps — hooks for state management |
| Flutter | `open_wallet` | iOS/Android native apps |
